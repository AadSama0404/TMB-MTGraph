...
class TMB_MTGraph_dataset(Dataset):
...
class total_tmb_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        _, sorted_indices = torch.sort(features[:, -1], descending=True)
        sorted_features = features[sorted_indices]
        sum_col1 = torch.sum(sorted_features[:, 0])
        weighted_avg = torch.zeros_like(sorted_features)
        for i in range(sorted_features.shape[0]):
            weighted_avg[i, 1] = sorted_features[i, 1] * sorted_features[i, 0] / sum_col1  # Proportional weighted average
            weighted_avg[i, 2] = sorted_features[i, 2] * sorted_features[i, 0] / sum_col1  # Proportional weighted average
        return (weighted_avg, response)
