...
def Separate_MLP_Train(train_loader, models, optimizers, fold):
    '''
...
def Cross_Validation(raw_data):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(raw_data, labels)):
        ...
        train_subset = Oversampling(oversample_rates[fold], train_subset_raw)
        ...
        for epoch in range(epochs[fold]):
            ...
            Separate_MLP_Train(train_loader, models, optimizers, fold)
            Separate_MLP_Val(val_loader, models, epoch==epochs[fold]-1, fold)
    ...
    result_cv.to_csv('../results/Separate_MLP.csv', index=False)
    Metrics_Calculation('../results/Separate_MLP.csv')
    KM_Plot('../results/Separate_MLP.csv')
